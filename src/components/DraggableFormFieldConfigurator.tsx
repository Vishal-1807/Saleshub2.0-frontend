...
interface DraggableFormFieldConfiguratorProps {
  field: FormField;
...
export function DraggableFormFieldConfigurator({
  field,
  onUpdate,
  onRemove,
  fieldTypes,
  ...
  return (
    <div
      ...
      style={style}
      className={`
        border border-slate-300 rounded-lg overflow-hidden bg-white transition-all duration-200
        ${isSortableDragging ? 'shadow-2xl scale-105 z-50' : 'shadow-sm hover:shadow-md'}
    ...
    >
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 p-3 bg-slate-50">
        ...
        <div
          {...attributes}
          {...listeners}
          className="flex items-center justify-center p-1 hover:bg-slate-200 rounded cursor-grab active:cursor-grabbing transition-colors flex-shrink-0 self-start sm:self-center"
